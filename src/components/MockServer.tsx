...
export const MockServer: React.FC = () => {
  const [rules, setRules] = useState<MockRule[]>([
    { id: "1", path: "/api/hello", method: "GET", status: 200, headers: [], body: '{"message": "Hello from Mock!"}' }
  ]);
  const [status, setStatus] = useState<MockServerStatus>({ is_running: false, port: 3000, active_rules: 0 });
  ...
  const handleStart = async () => {
    try {
      ...
      toast.error("Erro ao iniciar server", { description: e });
    }
  };
