...
import type { Collection, CollectionItem } from '../types/ipc';
import type { DesignSpec, HttpRequest } from '../types/ipc';
...
export function Sidebar() {
  ...
  const [sidebarTab, setSidebarTab] = useState<
    'Collections' | 'History' | 'Mock' | 'Monitor' | 'Designs'
  >('Collections');
      ...
      return (
        <div key={idx} className="sidebar-folder">
              ...
              <button
                ...
                onClick={() => {
                  window.dispatchEvent(
                    new CustomEvent('open-runner', { detail: item.Folder.items }),
                  );
                }}
              >
          ...
          <div className="sidebar-folder-items">
            {item.Folder.items.map((subItem: CollectionItem, subIdx: number) =>
              renderItem(subItem, subIdx, colId),
            )}
          </div>
        </div>
      );
        ...
        >
          <div className="sidebar-request-info">
            ...
            <span
              ...
              data-method={typeof item.Request.method === 'string' ? item.Request.method : 'CUSTOM'}
            >
              {typeof item.Request.method === 'string'
                ? item.Request.method
                : `CUSTOM (${item.Request.method.CUSTOM})`}
            </span>
            <span className="request-url" title={item.Request.url}>
              {item.Request.url}
            </span>
          </div>
          <div className="sidebar-item-actions">
  ...
  return (
    <aside className="sidebar">
      ...
      {sidebarTab === 'Collections' ? (
        ...
        <>
          ...
          <div className="sidebar-content">
            ...
            {history.map((req: HttpRequest, idx: number) => (
              <motion.div
                key={req.id + idx}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
              ...
              >
                <div className="sidebar-request-info">
                  ...
                  <span
                    className="method-badge"
                    data-method={typeof req.method === 'string' ? req.method : 'CUSTOM'}
                  >
        ...
        <>
          ...
          <div className="sidebar-content">
            {designs.length === 0 && (
              <div className="empty-state">No designs found. Create one to begin.</div>
            )}
            {designs.map((design: DesignSpec) => (
              <motion.div
                key={design.id}
                initial={{ opacity: 0 }}
                animate={{ opacity: 1 }}
      ...
      {showGitPanel && workspacePath && <GitPanel onClose={() => setShowGitPanel(false)} />}
    </aside>
  );
}
