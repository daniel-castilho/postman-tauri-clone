...
import {
  GitBranch, X, FileJson, Plus, CheckCircle2, AlertTriangle,
  ChevronDown, Undo2,
} from 'lucide-react';
...
export const GitPanel: React.FC<Props> = ({ onClose }) => {
  ...
  const openDiff = async (change: GitFileChangeDto) => {
    try {
      setDiff(await invoke<GitFileDiffDto>('git_get_file_diff', {
        workspacePath,
        file: change.path,
  ...
  const handleCommit = async () => {
    ...
    const message = description.trim()
      ...
      : summary.trim();
    await runAction(
      () => invoke('git_commit', { workspacePath, message }),
      `Committed`,
    );
    setSummary('');
    setDescription('');
      ...
      <div className="modal-content git-panel-modal" onClick={e => e.stopPropagation()}>
        ...
        {status && !status.is_repository && (
          <div className="git-empty">
            ...
            <p>This workspace is not a Git repository yet.<br />Run <code>git init</code> to enable version control.</p>
          </div>
        )}
        ...
        {status?.is_repository && (
          <div className="modal-body-content git-panel-body">
            ...
            <div className="git-branch-row">
              <button className="git-branch-switcher" onClick={() => setBranchMenuOpen(open => !open)}>
                <GitBranch size={13} />
            ...
            <div className="git-files-section">
              ...
              {staged.map(change => (
                <div key={`${change.path}-staged`} className="git-file-row">
                  <button className="git-file-name" onClick={() => openDiff(change)} title="View diff">
                    <FileJson size={13} /> {change.path}
                  </button>
                  <span className={`git-status-badge git-status-${change.status}`}>{STATUS_LABELS[change.status]}</span>
                  <button
                    className="git-file-action"
                    disabled={busy}
                    onClick={() => runAction(() => invoke('git_unstage_file', { workspacePath, file: change.path }))}
                    title="Unstage"
                  >
                    <Undo2 size={13} />
                  </button>
              ...
              {unstaged.map(change => (
                <div key={`${change.path}-unstaged`} className="git-file-row">
                  ...
                  <button
                    className="git-file-action"
                    disabled={busy}
                    onClick={() => runAction(() => invoke('git_stage_file', { workspacePath, file: change.path }))}
                    title="Stage"
                  >
                    <Plus size={13} />
                  </button>
