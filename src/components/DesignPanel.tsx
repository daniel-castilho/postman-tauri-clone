...
import { useWorkspaceStore, DesignSpec, LintIssue } from '../store/workspaceStore';
...
export const DesignPanel: React.FC = () => {
  const { 
    designs, activeDesignId, saveDesign, loadDesigns, 
    createDesign, deleteDesign 
  } = useWorkspaceStore();
        ...
        <div className="lint-panel">
           <div className="lint-header">
              ...
              <div className="lint-stats">
                 <span className="error"><AlertTriangle size={12} /> {lintIssues.filter(i => i.severity === 'Error').length}</span>
                 <span className="warning"><Info size={12} /> {lintIssues.filter(i => i.severity === 'Warning').length}</span>
              </div>
           </div>
