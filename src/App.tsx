...
import { Save, Settings, Terminal, Copy, Check, Sun, Moon, Sparkles, BrainCircuit, Code2, Globe, Users, Share2 } from "lucide-react";
import { Toaster, toast } from 'sonner';
import { motion, AnimatePresence } from 'framer-motion';
import { useShortcuts } from "./hooks/useShortcuts";
import { TauriService } from "./api/tauriService";
import "./App.css";

function App() {
  const { 
    workspacePath, activeRequest, updateRequest, environments, 
    activeEnvironmentId, setActiveEnvironment, updateEnvironment, 
    addToHistory, activeTabId, globals, sessionVariables, saveGlobals,
    sidebarMode 
  } = useWorkspaceStore();
  const [runnerItems, setRunnerItems] = useState<any[] | null>(null);
  ...
  useEffect(() => {
    const handler = (e: any) => setRunnerItems(e.detail);
    window.addEventListener('open-runner', (e: any) => setRunnerItems(e.detail));
    return () => window.removeEventListener('open-runner', handler);
  }, []);
  const activeEnvironment = environments?.find((e: any) => e.id === activeEnvironmentId) || null;
  ...
  const [formData, setFormData] = useState<{key: string, value: string, file: string | null, enabled: boolean}[]>([]);
  const [gqlQuery, setGqlQuery] = useState("");
  const [gqlVariables, setGqlVariables] = useState("");
  const [auth, setAuth] = useState<any>("NoAuth");
  const [activeConfigTab, setActiveConfigTab] = useState<'Params' | 'Headers' | 'Body' | 'Auth' | 'Scripts' | 'Docs' | 'LoadTest'>('Body');
  const [activeResponseTab, setActiveResponseTab] = useState<'Body' | 'Headers' | 'Preview' | 'Test Results' | 'Console'>('Body');
  ...
  useEffect(() => {
    if (activeRequest) {
      ...
      setAuth(activeRequest.auth || "NoAuth");
  ...
  async function handleSend() {
    setLoading(true);
    setResponse("Disparando requisição via Rust...");
    
    try {
      let backendBody: any = null;
      if (bodyMode === 'Raw' && body) {
         backendBody = { Raw: [body, "Json"] };
  ...
  const generateCurl = () => {
    ...
    if (auth && typeof auth !== 'string' && auth.Bearer) {
  ...
  return (
    <div className="app-layout">
      ...
      <div className="main-content-area">
          ...
          <div className="header-right">
            <div className="env-selector">
              <select 
                ...
                onChange={(e: React.ChangeEvent<HTMLSelectElement>) => setActiveEnvironment(e.target.value)}
                className="env-select"
              >
                {environments.map((e: Environment) => <option key={e.id} value={e.id}>{e.name}</option>)}
              </select>
              <button 
              ...
              <button className="env-settings-btn" onClick={() => setShowGlobalsModal(true)} title="Manage Globals">
                <Globe size={16} />
              </button>
              <button className="env-settings-btn" onClick={() => setShowEnvModal(true)} title="Manage Environments">
                <Settings size={16} />
              </button>
              <button 
        ...
        {sidebarMode === 'Designs' ? (
          ...
          <AnimatePresence mode="wait">
            ...
            <button 
              ...
              onClick={async () => {
                ...
                const code = await invoke<string>("generate_js_code", { request, target: codeTarget });
            ...
            <div className="cookies-overlay" onClick={() => setShowCookies(false)}>
              <div className="cookies-modal" onClick={e => e.stopPropagation()}>
                <div className="cookies-modal-header">
                  ...
                  <button className="close-cookies-btn" onClick={() => setShowCookies(false)}>×</button>
                </div>
          ...
          {url.includes('{{') && (() => {
            ...
            envVars.forEach(v => { if (v.enabled) varMap[v.key as string] = v.current_value; });
            ...
            return (
              <div className="var-hint-bar">
                {matches.map((m, i) => {
                  ...
                  return (
                    <span key={i} className={`var-token ${resolved ? 'resolved' : 'unresolved'}`}>
          ...
          <div className="request-config-panel">
            <div className="tabs">
              <button className={`tab-btn ${activeConfigTab === 'Params' ? 'active' : ''}`} onClick={() => setActiveConfigTab('Params')}>
                Params ({params.length})
              </button>
              <button className={`tab-btn ${activeConfigTab === 'Auth' ? 'active' : ''}`} onClick={() => setActiveConfigTab('Auth')}>
                Auth
              </button>
              <button className={`tab-btn ${activeConfigTab === 'Headers' ? 'active' : ''}`} onClick={() => setActiveConfigTab('Headers')}>
                Headers ({headers.length})
              </button>
              <button className={`tab-btn ${activeConfigTab === 'Body' ? 'active' : ''}`} onClick={() => setActiveConfigTab('Body')}>
                Body
              </button>
              <button className={`tab-btn ${activeConfigTab === 'Scripts' ? 'active' : ''}`} onClick={() => setActiveConfigTab('Scripts')}>
                Scripts
              </button>
              <button className={`tab-btn ${activeConfigTab === 'Docs' ? 'active' : ''}`} onClick={() => setActiveConfigTab('Docs')}>
                Documentation
              </button>
              <button className={`tab-btn ${activeConfigTab === 'LoadTest' ? 'active' : ''}`} onClick={() => setActiveConfigTab('LoadTest')}>
                Load Test
              </button>
            </div>
            ...
            <div className="tab-content">
              {activeConfigTab === 'Params' && (
                <div className="headers-pane">
                        ...
                        onChange={(e) => {
                          ...
                          try {
                            ...
                            newParams.forEach(np => {
                              if (np.enabled && np.key) urlObj.searchParams.append(np.key, np.value);
                            });
                  ...
                  <button onClick={() => setParams([...params, { key: '', value: '', enabled: true }])} className="add-row-btn">
                    + Add Parameter
                  </button>
                </div>
              )}
                ...
                <div className="auth-pane">
                  <select 
                    ...
                    onChange={(e) => {
                      ...
                      else if (type === 'Bearer') setAuth({ type: 'Bearer', data: { token: '' } });
                      else if (type === 'Basic') setAuth({ type: 'Basic', data: { username: '', password: '' } });
                      else if (type === 'ApiKey') setAuth({ type: 'ApiKey', data: { key: 'x-api-key', value: '', in_header: true } });
                      else if (type === 'OAuth2') setAuth({ type: 'OAuth2', data: { access_token: '', header_prefix: 'Bearer' } });
                      else if (type === 'AWSSig4') setAuth({ type: 'AWSSig4', data: { access_key: '', secret_key: '', region: 'us-east-1', service: 'execute-api' } });
                    }}
                  ...
                  <div className="auth-inputs">
                    {auth?.type === 'Bearer' && (
                       <input 
                         ...
                         onChange={(e) => setAuth({ ...auth, data: { ...auth.data, token: e.target.value } })}
                       />
                    )}
                    {auth?.type === 'Basic' && (
                       <>
                    ...
                    {auth?.type === 'ApiKey' && (
                       <>
                         <input 
                           ...
                           placeholder="Key" 
                           value={auth.data?.key || ''}
                           onChange={(e) => setAuth({ ...auth, data: { ...auth.data, key: e.target.value } })}
                         />
                         <input 
                           type="text" 
                           placeholder="Value" 
                           value={auth.data?.value || ''}
                           onChange={(e) => setAuth({ ...auth, data: { ...auth.data, value: e.target.value } })}
                         />
                         <select 
                            value={auth.data?.in_header ? "header" : "query"}
                            onChange={(e) => setAuth({ ...auth, data: { ...auth.data, in_header: e.target.value === "header" } })}
                    ...
                    {auth?.type === 'OAuth2' && (
                       <input 
                         ...
                         onChange={(e) => setAuth({ ...auth, data: { ...auth.data, access_token: e.target.value } })}
                       />
                    )}
                    {auth?.type === 'AWSSig4' && (
                       <div className="auth-note">AWS Signature v4 structure defined. Full signing logic pending.</div>
                    )}
                  </div>
                </div>
              )}
                ...
                <div className="headers-pane">
                  {headers.map((h, idx) => (
                    <div key={idx} className="header-row">
                      ...
                      />
                      <button onClick={() => setHeaders(headers.filter((_, i) => i !== idx))} className="remove-row-btn">×</button>
                    </div>
                  ))}
                  <button onClick={() => setHeaders([...headers, { key: '', value: '', enabled: true }])} className="add-row-btn">
                    + Add Header
                  </button>
                </div>
              )}
                  ...
                  <div className="script-section">
                    <div className="script-header">
                      ...
                      <button 
                        ...
                        onClick={async () => {
                          ...
                          try {
                            ...
                            toast.success("Testes gerados pela IA!");
                             ...
                             toast.error("IA Falhou", { description: e });
                          } finally {
                            setAiLoading(false);
                          }
                        }}
                        disabled={aiLoading}
                      >
                         {aiLoading ? "Gerando..." : <><Sparkles size={14} /> Gerar Testes com IA</>}
                      </button>
                    </div>
                    <textarea 
                      value={testScript}
                    ...
                    <>
                      <div className="body-toolbar">
                        <div className="body-mode-selector">
                          <button className={`mode-btn ${bodyMode === 'None' ? 'active' : ''}`} onClick={() => setBodyMode('None')}>none</button>
                          <button className={`mode-btn ${bodyMode === 'Raw' ? 'active' : ''}`} onClick={() => setBodyMode('Raw')}>raw</button>
                          <button className={`mode-btn ${bodyMode === 'FormData' ? 'active' : ''}`} onClick={() => setBodyMode('FormData')}>form-data</button>
                          <button className={`mode-btn ${bodyMode === 'GraphQL' ? 'active' : ''}`} onClick={() => setBodyMode('GraphQL')}>graphql</button>
                        </div>
                        {bodyMode === 'Raw' && (
                          <button onClick={() => {
                              try {
                          ...
                          }} className="beautify-btn">Beautify</button>
                        )}
                      </div>
                            ...
                            <div key={idx} className="header-row">
                              ...
                              {f.file !== null ? (
                                <div className="file-input-wrapper">
                                  <input type="text" readOnly value={f.file || "No file selected"} className="file-path-input" />
                              ...
                              <button onClick={() => setFormData(formData.filter((_, i) => i !== idx))} className="remove-row-btn">×</button>
                            </div>
                          ))}
                          <button onClick={() => setFormData([...formData, { key: '', value: '', file: null, enabled: true }])} className="add-row-btn">
                            + Add Form Field
                          </button>
                        </div>
                      )}
                ...
                <div className="tab-content body-tab">
                  ...
                  <div className="docs-editor-container" style={{ padding: '16px' }}>
                    <textarea 
                      ...
                      style={{ height: '300px', width: '100%', resize: 'vertical', fontSize: '14px', lineHeight: '1.6' }}
                    />
                  </div>
                </div>
              )}

              {activeConfigTab === 'LoadTest' && (
                <LoadTestingPanel />
              )}
            </div>
          </div>
            ...
            <div className="response-header">
              <div className="response-header-left">
                ...
                <div className="response-tabs">
                  ...
                  >Headers {response?.headers?.length > 0 && `(${response.headers.length})`}</button>
                  <button 
                  ...
                  >
                    Test Results 
                    {response?.testsResults?.length > 0 && (
                      <span className="test-count">
                        ({response.testsResults.filter((r: TestResult) => r.passed).length}/{response.testsResults.length})
                      </span>
                    )}
                  </button>
                  <button 
                  ...
                  >
                    Console 
                    {response?.logs?.length > 0 && ` (${response.logs.length})`}
                  </button>
                </div>
              </div>
              <div className="response-header-right">
                ...
                {response && response.status && (
                  <div className="response-meta-new">
                    <span className={`status-badge ${response.status >= 200 && response.status < 300 ? 'status-ok' : 'status-err'}`}>
                      {response.status} {response.statusText}
                    </span>
                    <span className="meta-item">{response.timeMs} ms</span>
                    <span className="meta-item">{(response.sizeBytes / 1024).toFixed(2)} KB</span>
                  </div>
                )}
              </div>
            </div>
                        ...
                        <div className="ai-explanation-bar">
                           <button 
                             ...
                             onClick={async () => {
                                setAiLoading(true);
                                try {
                                  const explanation = await invoke<string>("ai_explain_response", { responseBody: response.body });
                           ...
                           >
                             <BrainCircuit size={14} /> {aiLoading ? "Analisando..." : "Explicar com IA"}
                           </button>
                           {aiExplanation && (
                             <div className="ai-explanation-content">
                                {aiExplanation}
                                <button onClick={() => setAiExplanation("")} className="close-ai-btn">×</button>
                             </div>
                           )}
                        </div>
                      )}
                      <pre className="response-body" dangerouslySetInnerHTML={{ 
                        __html: responseMode === 'Pretty' 
                  ...
                  {activeResponseTab === 'Headers' && (
                    <div className="response-headers-list">
                      <table className="headers-table">
                        ...
                        <tbody>
                          {response.headers && response.headers.map((h: any, i: number) => (
                            <tr key={i}>
                  ...
                  {activeResponseTab === 'Test Results' && (
                    <div className="test-results-view">
                      {!response.testsResults || response.testsResults.length === 0 ? (
                        <div className="empty-tests-msg">Nenhum teste configurado ou executado para esta requisição.</div>
                      ) : (
                        <div className="test-results-list">
                          {response.testsResults.map((r: TestResult, i: number) => (
                            <div key={i} className={`test-result-item ${r.passed ? 'passed' : 'failed'}`}>
                              <span className="test-status-badge">{r.passed ? 'PASS' : 'FAIL'}</span>
                  ...
                  {activeResponseTab === 'Console' && (
                    <div className="test-results-view">
                      {!response.logs || response.logs.length === 0 ? (
                        <div className="empty-tests-msg">Nenhum log gerado pelos scripts. Use console.log() para depurar.</div>
                      ) : (
        ...
        <div className="modal-overlay">
          <div className="modal-content">
            <div className="modal-header">
              ...
              <button onClick={() => setShowEnvModal(false)} className="close-modal-btn">×</button>
            </div>
            <div className="modal-body">
              <div className="env-vars-list">
                {Object.entries(activeEnvironment.variables).map(([key, val], idx) => (
                  <div key={idx} className="header-row">
                    <input 
                      type="text" 
                      value={String(key)}
                      ...
                      onChange={(e) => {
                        const newVars: any = { ...activeEnvironment.variables };
                        delete newVars[key];
                ...
                <button className="add-row-btn" onClick={() => {
                  const newVars = { ...activeEnvironment.variables, [`NewVar_${Date.now()}`]: "" };
                  updateEnvironment({ ...activeEnvironment, variables: newVars });
                }}>
                  + Add Variable
                </button>
        ...
        <div className="modal-overlay">
          <div className="modal-content curl-modal">
            <div className="modal-header">
              ...
              <button onClick={() => setShowCurlModal(false)} className="close-modal-btn">×</button>
            </div>
            <div className="modal-body">
              <div className="curl-container">
      ...
      {showGlobalsModal && (
        <GlobalsModal onClose={() => setShowGlobalsModal(false)} />
      )}
