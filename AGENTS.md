...
2. **OpenAPI 3.1 Spec Schema Expansion:** Resolved — `lint_spec` now parses the document (JSON or YAML), detects the dialect from the `openapi` field and applies 3.1-specific JSON Schema (2020-12) rules: missing/unknown `jsonSchemaDialect`, removed `nullable` keyword, boolean `exclusiveMinimum`, singular `example`, `$dynamicRef`/`$dynamicAnchor` tooling warnings — with structural responses/servers checks shared by 3.0 and 3.1. Covered by 13 unit tests in `design_tasks.rs`.
3. **Workspace File Watcher:** Resolved — `WorkspaceWatcherPort` + `notify` adapter (`infrastructure/watcher/notify_adapter.rs`) watch the workspace recursively, debounce events (300 ms), filter sensitive files, and stream batches through the `workspace-changed` event (`start/stop_workspace_watch` commands). The store reloads collections/environments/globals/designs while preserving open tabs and unsaved drafts.
4. **Manual Binding Registration:** Resolved via a dependency-free drift guard — `export_list_covers_every_ts_annotated_type` (`export_ts_bindings.rs`) statically parses `domain/models.rs` and fails `cargo test` when any `#[ts(export)]` type is missing from the export list. Verified to catch drift end-to-end; a proc-macro inventory remains unnecessary.
5. **UI Strings Still Portuguese:** Resolved — every user-facing React string and the AI prompt strings in `gemini_adapter.rs` are now English (Rule 6 fully satisfied).
...
8. **Consultative React Hooks Rules:** Resolved — data-loading effects were refactored into guarded loaders / adjust-state-during-render patterns across all panels, and `react-hooks/set-state-in-effect`, `react-hooks/immutability` and `react-hooks/exhaustive-deps` now run as **errors** in ESLint (`eslint.config.mjs`).
9. **Coverage gated surfaces:** Partially resolved — the Vitest gate now instruments `src/lib/**`, `src/store/**`, `src/hooks/**` and the first tested components (LoadTestCharts, LoadTestReport, WorkspaceSelector), holding **96.9% lines / 88.5% branches / 99.2% functions**; the Rust side added port-level tests for the scripting engine (`execute_pre_request`/`execute_test` through `ScriptRunnerPort`) plus suites for markdown docs, codegen templates, OpenAPI importer and AES encryption, letting the cargo-llvm-cov regex shrink to only genuinely untestable surfaces (rationale inline in `scripts/coverage-rust.sh`) at **87.9% lines / 84.1% functions**. Remaining increments: more `src/components/**` render tests; remaining infrastructure adapters. Do not lower the 80% floors.
