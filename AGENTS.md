...
9. **Coverage gated surfaces:** Partially resolved — the Vitest gate now instruments `src/lib/**`, `src/store/**` and `src/hooks/**` (96.6% lines / 87.4% branches), and the cargo-llvm-cov regex shrank to only genuinely untestable surfaces (Tauri-harness commands, bootstrap, derive boilerplate — rationale documented inline in `scripts/coverage-rust.sh`), holding 87.9% lines / 84.1% functions. Remaining increments: `src/components/**` render tests on the frontend; `quickjs_runner` + remaining infrastructure adapters on the Rust side. Do not lower the 80% floors.
