...
- [x] **Collection Runner Exportable Reports**: shared Rust renderers (`html_reporter`, `markdown_reporter`) power both the GUI export buttons (via the thin `render_run_report` IPC command) and the headless CLI (`--format html` / `.html` extension). Self-contained styled HTML with escaped dynamic content; covered by renderer unit tests, CLI format-resolution tests and component flow tests.
- [x] **Real-Time Workspace Watcher (debt #3)**: `WorkspaceWatcherPort` + `notify` adapter with 300 ms debounce, JSON-only relevance filtering and secret-file exclusions; `workspace-changed` events drive silent reloads of collections/environments/globals/designs while preserving open tabs and drafts.
